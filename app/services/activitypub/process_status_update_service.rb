...
class ActivityPub::ProcessStatusUpdateService < BaseService
  ...
  def update_tagged_objects!
    current_tagged_objects = @raw_tagged_objects.flat_map do |tagged_object|
    ...
    @status.tagged_objects.upsert_all(
      current_tagged_objects.uniq.map { |object| { object_type: object.class.name, object_id: object.id, uri: ActivityPub::TagManager.instance.uri_for(object) } },
